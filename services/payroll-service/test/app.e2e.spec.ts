...
describe('payroll-service bootstrap', () => {
  ...
  beforeAll(async () => {
    const testingModule = await Test.createTestingModule({
      ...
      .useValue({ check: async () => ({ database: 'up', redis: 'up' }) })
      .compile();
  ...
  it.each(['/health/live', '/health/ready'])(
    ...
    async (path) => {
      ...
      expect(response.body).toEqual({
        ...
        ...(path.endsWith('/ready') ? { dependencies: { database: 'up', redis: 'up' } } : {}),
      });
    },
  );
  ...
  it('exposes low-cardinality Prometheus metrics', async () => {
    ...
    expect(response.text).toContain('service="payroll-service"');
  });
