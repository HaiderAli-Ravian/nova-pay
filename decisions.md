...
NovaPay uses six independently packaged NestJS services in one npm-workspace monorepo: Account, Transaction, Ledger, FX, Payroll, and Admin. The services use native ESM, share one deterministic root lockfile, and retain independent package versions, tests, and Docker images. Account and Ledger are currently `0.3.0`; the remaining services are `0.2.0`. The boundaries match data ownership and assessment responsibilities without adding a runtime shared package.
